...
public class ReachAnalysis
{
	...
	public ReachAnalysis(Traversable icode, CFGraph icfg, Set<Symbol> tarvars)
	{
		input_code = icode;
		cfg = icfg;
		target_vars = tarvars;
		...
		if ( o instanceof Traversable )
		{
			...
			for ( Expression e : DataFlowTools.getDefSet(tr) )
			{
				...
				if (def_symbol != null && target_vars.contains(def_symbol) )
				{
					...
					out = out.unionWith(AnalysisTools.getDefSectionMap(e, rd, defined_vars), rd);
				}
			}

			//modified_vars.addAll(DataFlowTools.getDefSymbol(tr));
			modified_vars.addAll(AnalysisTools.getBaseSymbols(DataFlowTools.getDefSymbol(tr)));
