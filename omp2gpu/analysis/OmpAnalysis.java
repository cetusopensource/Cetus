...
public class OmpAnalysis extends AnalysisPass
{
	...
	public void start()
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////
		// Analysis passes in omp2gpu assume that omp parallel region is either CompoundStatement    //
		// or ForLoop. However, in OpenMP C/C++ specification, an executable statement with a single //
		// entry at the top and a single exit at the bottom can constitute a parallel region.        //
		// For analysis passes to handle the single statement case, a "single-statement" parallel    //
		// region is replaced with a CompoundStatement where the single statement is contained.      //
		///////////////////////////////////////////////////////////////////////////////////////////////
		List<OmpAnnotation> ompParallelAnnots = 
			IRTools.collectPragmas(program, OmpAnnotation.class, "parallel");
		for( OmpAnnotation pAnnot : ompParallelAnnots ) {
		...
		/*
		 * Omp threadprivate directive is not used as a clause for omp parallel 
		 * directives. To assist other analysis, however, we insert
		 * threadprivate data list into omp parallel constructs. 
		 * CAUTION: For correct OpenMP program, these threadprivate clauses added in 
		 * the omp parallel constructs should be removed later.
		 */
		/////////////////////////////////////////////
		// Collect OpenMP threadprivate variables. //
		/////////////////////////////////////////////
		threadprivSet = new HashSet<String>();
		...
		// return the same output. Therefore, below HashSet<OmpAnnotation> should be replaced with     //
		// LinkedList<OmpAnnotation>.                                                                  //
		/////////////////////////////////////////////////////////////////////////////////////////////////
		LinkedList<OmpAnnotation> OmpPAnnotList = new LinkedList<OmpAnnotation>();
		OmpPAnnotList.addAll( IRTools.collectPragmas(program, OmpAnnotation.class, "parallel") );
		// DEBUG: threadprivate set in an omp-for loop is not used for analysis; comment it out.
		//OmpPAnnotList.addAll( IRTools.collectPragmas(program, OmpAnnotation.class, "for") );
		for( OmpAnnotation annot : OmpPAnnotList ) {
	...
	{ 
		...
		Set<Symbol> ForAccessedSet = AnalysisTools.getAccessedVariables(loop);
	...
	private HashSet<Symbol> findSharedInRegion(OmpAnnotation omp_annot)
	{
		...
		Set<Symbol> AccessedSet = AnalysisTools.getAccessedVariables(annot_container);
		...
		if ( annot_container instanceof SymbolTable ) {
			...
			// Remove local procedure symbols from LocalPrivSet. //
			Set<Symbol> tSet = new HashSet<Symbol>();
			for( Symbol sm : LocalPrivSet ) {
				if( (sm instanceof Procedure) || (sm instanceof ProcedureDeclarator) ) {
					tSet.add(sm);
				}
			}
			LocalPrivSet.removeAll(tSet);
			Set<Symbol> StaticLocalSet = AnalysisTools.getStaticVariables(LocalPrivSet);
...
*/
		...
		// add all accessed variable symbols in the procedure
		Set<Symbol> accessedSymbols = AnalysisTools.getAccessedVariables(proc_body);
		...
		// ==> threadprivate set is recalculated at every function and added to the         //
		// threadprivate set in the enclosing parallel regioin.                             //
		// FIXME: in the below conversion, LOmpThreadPrivSet may contain parameter symbols  //
		// of the current procedure; the attributes of the parameter symbols should inherit //
		// the attributes of corresponding argument symbols, but not handled here.          //
		// ==> parameter symbols are removed from LOmpThreadPrivSet.                        //
		//////////////////////////////////////////////////////////////////////////////////////
		HashSet<Symbol> OmpThreadPrivSet = (HashSet<Symbol>)omp_annot.get("threadprivate");
		HashSet<Symbol> LOmpThreadPrivSet = convertString2Symbol(threadprivSet, proc_body);
		Set<Symbol> paramSyms = SymbolTools.getParameterSymbols(proc);
		LOmpThreadPrivSet.removeAll(paramSyms);
		LOmpThreadPrivSet.removeAll(LocalPrivSet);
		displaySet("threadprivate variables in a procedure", LOmpThreadPrivSet);
		///////////////////////////////////////////////////////////////////////////////////
		// Add locally found threadprivate set to the threadprivate set of the enclosing //
		// parallel region.                                                              //
		///////////////////////////////////////////////////////////////////////////////////
		OmpThreadPrivSet.addAll(LOmpThreadPrivSet);
		LocalPrivSet.removeAll(OmpThreadPrivSet);
	...
	private HashSet<Symbol> convertString2Symbol(Set<String> iset, Statement stmt)
	{
		...
		Set<Symbol> accessed_set = AnalysisTools.getAccessedVariables(stmt);
...
/*	static private String[] omp_pragma_wo_attach = {
	...
	static public void updateReductionClause(Traversable t, OmpAnnotation omp_map)
	{
		...
		// Remove local procedure symbols from accessedSymbols. //
		Set<Symbol> tSet = new HashSet<Symbol>();
		for( Symbol sm : accessedSymbols ) {
			if( (sm instanceof Procedure) || (sm instanceof ProcedureDeclarator) ) {
				tSet.add(sm);
